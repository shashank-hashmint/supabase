...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Get the file record to verify ownership and get file details
    const { data: fileRecord, error: fetchError } = await supabaseClient
      .from('user_pdfs')
      .select('id, s3_key, original_filename, file_size, content_type')
      .eq('id', fileId)
      .eq('user_id', user.id)
      .single();
    ...
    // Get AWS credentials for generating download URL
    ...
    return new Response(JSON.stringify({
      ...
      data: {
        ...
        contentType: fileRecord.content_type || 'application/pdf'
      }
    }), {
      headers: {
        ...corsHeaders, 
...
// Helper function to generate pre-signed download URL using AWS Signature Version 4
async function generateDownloadUrl(params: {
  ...
  const baseUrl = `https://${host}/${objectKey}`;
